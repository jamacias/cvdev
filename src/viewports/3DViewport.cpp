...
ThreeDViewport::ThreeDViewport(const Magnum::Vector2i& size, const std::shared_ptr<Scene3D> scene)
: size_(size)
, scene_(scene)
{
    ...
    camera_->setProjectionMatrix(Matrix4::perspectiveProjection(
        45.0_degf, Vector2{size_}.aspectRatio(), 0.01f, 100.0f));

    lastDepth_ = ((camera_->projectionMatrix() * camera_->cameraMatrix()).transformPoint({}).z() + 1.0f) * 0.5f;

    framebuffer_ = Magnum::GL::Framebuffer{{{}, size}};
    colorTexture_ = Magnum::GL::Texture2D{};
    colorTexture_.setStorage(1, Magnum::GL::TextureFormat::RGBA8, size);
    depthBuffer_ = Magnum::GL::Renderbuffer{};
    depthBuffer_.setStorage(Magnum::GL::RenderbufferFormat::DepthComponent24, size);
    framebuffer_.attachTexture(Magnum::GL::Framebuffer::ColorAttachment{0}, colorTexture_, 0)
                .attachRenderbuffer(Magnum::GL::Framebuffer::BufferAttachment::Depth, depthBuffer_)
                .mapForDraw({{0, Magnum::GL::Framebuffer::ColorAttachment{0}}});
...
void ThreeDViewport::draw(SceneGraph::DrawableGroup3D& drawables)
{
    ...
    framebuffer_.bind();
    framebuffer_.clear(Magnum::GL::FramebufferClear::Color|Magnum::GL::FramebufferClear::Depth);
    ...
    ImGui::Image(static_cast<ImTextureID>(colorTexture_.id()), ImVec2(size_.x(), size_.y()),
                 ImVec2(0, 1), ImVec2(1, 0) // UV coordinates (flip Y if needed)
    );
    ImGui::End();
}